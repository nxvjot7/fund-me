...
contract FundFundMe is Script {
    uint256 constant SEND_VALUE = 0.1 ether; //send_value being set to 0.1 ether

    function run() public {
        //when we call run() it automatically retrieves data of most recently deployed contract from broadcast folder
    ...
    function fundFundME(address mostRecentlyDeployedFundMe) public {
        //asking for address of latest Deployed FundMe contract which is being passed from DeployFundMe.s.sol
...
contract WithdrawFundMe is
    Script //new contract for withdrawing funds
{
    function run() public {
        //when we call run() it automatically retrieves data of most recently deployed contract from broadcast folder
    ...
    function withdrawFundMe(address mostRecentlyDeployedFundMe) public {
        //asks for most recently deployed fundMe contract's address
        vm.startBroadcast();
