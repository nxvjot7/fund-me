...
contract FundMeTest is Test {
    ...
    function setUp() external {
        // Foundry test lifecycle: Snapshot state -> run setUp() -> execute test -> revert to snapshot (repeats for every test) same loop for next function test
    ...
    function testPriceFeedVersion() external view {
        uint256 version = fundMe.getVersion(); //calling GetVersion() from fundMe contract and storing its value in version variable
    ...
    modifier funded() {
        //modifier that automatically funds fund() from fundMe contract with 2 ether(SEND_VALUE) avoiding repititive coding
        vm.prank(USER); //tells to send next transaction from USER address
        fundMe.fund{value: SEND_VALUE}(); //funded by "USER" with 2 ether(SEND_VALUE)
    ...
    function testWithdrawFromMultipleFunders() public funded {
        ...
        for (
            uint256 i = startingFunderIndex; //created a for loop to fund the contract from multiple addresses, starting from 2 because 0 and 1 are used in other tests, and we want to avoid confusion
            i < numbersOfFunders;
            i++
        ) {
